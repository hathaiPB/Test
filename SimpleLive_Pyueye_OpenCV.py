...
while(nRet == ueye.IS_SUCCESS):
    ...
    frame_origin = np.reshape(array,(height.value, width.value, bytes_per_pixel))
    ...
    frame = cv2.resize(frame_origin,(0,0),fx=0.2, fy=0.2)
...
#---------------------------------------------------------------------------------------------------------------------------------------
    ...
    image_gray = cv2.cvtColor(frame_origin, cv2.COLOR_RGB2GRAY)
    # Filters
    color_lower_bound = np.array([30, 150, 130])
    color_upper_bound = np.array([45, 256, 256])
    sobelxy = cv2.Sobel(src=image_gray, ddepth=cv2.CV_64F, dx=1, dy=1, ksize=5) # Combined X

    mask_color = cv2.inRange(image_blur_hsv, color_lower_bound, color_upper_bound)
#---------------------------------------------------------------------------------------------------------------------------------------

    # ...and finally display it
    cv2.imwrite('D:/Work2021/test/image_data/2/'+str(i)+'.jpg', frame_origin)
    i +=1
